...
7. Optional: install `media-issue-agent` on the same network after `media-mcp`.
   ...
   The issue agent refuses generic Codex API-key auth through `OPENAI_API_KEY` and `CODEX_API_KEY`; use the Web UI Codex Auth panel or run Codex login with ChatGPT auth instead. Approved server-side fixes run as autonomous Codex repair prompts with direct `media-mcp` access, live tool discovery, optional non-secret repair context, and per-job scratch workspaces, then return to the Web UI for final resolution-comment approval. The Improvement Backlog tracks missing MCP capabilities and reusable investigation-prompt lessons learned from operator steering; resolved issues can also be analyzed retroactively from the issue list. Set `ISSUE_AGENT_SERVER_OWNER_REPORTER_USERNAME` only when one exact reporter account should be treated as trusted operator guidance. To receive Pushover notifications for newly detected open issues, set `ISSUE_AGENT_PUSHOVER_APP_TOKEN` and `ISSUE_AGENT_PUSHOVER_USER_KEY`. An optional Socket Mode Slack bot can receive media reports and requests, hold basic conversations, and answer privacy-safe read-only questions about library counts, title status/storage, bandwidth, queues, subtitles, recent additions, and service health; Slack requires a restricted `ISSUE_AGENT_OPENAI_MODERATION_API_KEY` used only with the free Moderation endpoint. See `docs.md` and `media-issue-agent/slack-app-manifest.yml`. Never mount media or download folders into `media-issue-agent`; those mounts belong only to `media-mcp`.
