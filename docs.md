...
The `media-issue-agent` container must not be given media or download mounts. The supplied Docker Compose service and Unraid template mount only `/state` and `/codex-home`; direct media/download access belongs exclusively to `media-mcp`. Conversation classification has no media MCP access. Legitimate media questions receive concise conversational answers, while detected prompt injection receives a deterministic hostile rejection without executing a lookup.
